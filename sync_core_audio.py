...
print(f"Found {len(to_sync)} successfully generated passages to sync.")
...
for i, row in to_sync.iterrows():
    ...
    storage_path = f"audio/corpus/COR/{passage_id}.mp3"
...
sync_log_path = BASE_DIR / "audio" / "core_sync_log.csv"
...
if success_count < len(sync_log_df):
    print("⚠️  Some passages failed — check core_sync_log.csv for details before considering this batch complete.")
