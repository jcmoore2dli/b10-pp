...
import LarRecorder from '../components/LarRecorder'
...
// response shape, different stimulus. INT and LAR have their own recorders
// (four clips; one continuous recording). CTW and BAS are separately scheduled.
...
export default function ItemScreen() {
  ...
  if (taskType === 'LAR') {
    return <LarRecorder itemId={itemId} />
  }
  ...
  return (
    <StubScreen
      route={`/item/${itemId}`}
      title="Item"
      note={`taskType = ${taskType}. Renderer for this type is not built yet — only ${[...MCQ_TYPES, ...TYPED_TYPES, 'INT', 'LAR'].join(', ')} are live.`}
    />
  )
}
